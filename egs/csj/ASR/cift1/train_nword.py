...
def compute_loss(
    ...
    y2 = sp.encode(texts, out_type=int)
    y = k2.RaggedTensor(y2).to(device)

    target_lens = []
    for s, yy in zip(texts, y2):
        if not yy:
            target_lens.append(0)
            continue
        morpheme_count = s.count(" ") + 1
        ratio = morpheme_count / len(yy)
        if ratio < 0.2:
            ratio = 0.2
        target_lens.append(round(len(yy)*ratio))
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if batch_idx % params.log_interval == 0:
            ...
            if HAS_TELEGRAM and batch_idx in [0, 800] and not rank:
...
def run(rank, world_size, args):
    ...
    if rank == 0 and HAS_TELEGRAM and params.telegram_cred:
    ...
    lm = None # TargetLength.load(params.targetlen_from)
    ...
    train_cuts = csj_corpus.train_nosp_nosubset_nosil_cuts()
