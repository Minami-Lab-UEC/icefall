...
def main():
    ...
    # Gather stats
    for wblank in wblank_dir.glob("*.txt"):
        with wblank.open() as fin:
            for line in fin:
                line = line.strip().split("ref=")[1]
    ...
    plt.savefig(wblank_dir / "sym_per_awe_histogram6.png")

    # One-line csv
    with (wblank_dir / "symbol_per_awe6.tsv").open('w') as fout:
