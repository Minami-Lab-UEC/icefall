...
from lhotse.dataset import ReverbWithImpulseResponse, PerturbVolume
...
def make_cutset_blueprints(
    ...
    rever = ReverbWithImpulseResponse(
        p=0.9,
        preserve_id=True,
    )
    perturb_volume = PerturbVolume(
        p=0.5,
        preserve_id=True,
    )
    cut_sets = []
    ...
    for i in range(1, 4):
        ...
        cut_sets.append((f"eval{i}", rever(perturb_volume(cut_set))))
    ...
    cut_set = cut_set.trim_to_supervisions(keep_overlapping=False)
    cut_sets.append(("excluded", rever(perturb_volume(cut_set))))
    ...
    cut_set = cut_set.trim_to_supervisions(keep_overlapping=False)
    cut_sets.append(("valid",  rever(perturb_volume(cut_set))))
    ...
    train_set = rever(perturb_volume(train_set)) + rever(perturb_volume(train_set)).perturb_speed(0.9) + rever(perturb_volume(train_set)).perturb_speed(1.1)
